...
class TicketService {
  getTicketList(
    ...
    return tickets.map((t, index) => {
      ...
      const removeOwnerVet = () =>
        deleteFromList(
          `/api/v1/consultations/${t.consultation.id}/tickets/${t.id}`,
          t.id,
          [tickets, setTickets],
          [alerts, setAlerts],
          setMessage,
      ...
      if (auth === 'OWNER') {
        ...
        buttons =
          index === length - 1 &&
          t.user.authority.authority === 'VET' &&
          status !== 'CLOSED' ? (
            <ButtonGroup>
              <Button
                aria-label={'edit-' + t.id}
        ...
        <Row style={{ margin: '30px 0' }}>
          <Row>
            <h2 className="text-center">
              Consultation Number {consultation.id}
            </h2>
          </Row>
          <Row>
